...
def send_sms(*, to_number: str, body_text: str) -> SmsResult:
    ...
    if Client is None:
        return SmsResult(
            delivered=False, provider="twilio", detail="missing_dependency"
        )
    ...
    try:
        ...
        if messaging_service_sid:
            ...
            client.messages.create(
                to=normalized_number, from_=from_number, body=body_text
            )
        return SmsResult(delivered=True, provider="twilio")
    except Exception as e:
        return SmsResult(
            delivered=False, provider="twilio", detail=f"twilio_send_failed: {e}"
        )
