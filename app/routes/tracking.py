...
from fastapi import APIRouter, Depends, HTTPException, WebSocket, WebSocketDisconnect, status, Header, Request
...
from models import PilotLocation, Errand, User, IncidentReport, IncidentMessage, ErrandEvent
from notification_utils import notify_delay_detected

from business_metrics import observe_tracking_delay_detected, observe_tracking_update
...
def _tracking_status_payload(errand: Errand) -> dict:
    ...
    return {
        ...
        "window_start": window_info["window_start"].isoformat() if window_info["window_start"] else None,
        "window_end": window_info["window_end"].isoformat() if window_info["window_end"] else None,
        "reason": reason,
    }
...
def _build_location_response(location: PilotLocation, errand: Errand) -> LocationResponse:
    return LocationResponse(
        ...
        recorded_at=_coerce_utc(getattr(location, "recorded_at", None)) or _coerce_utc(location.created_at),
...
async def update_location(
    ...
    try:
        ...
        if exc.status_code in {status.HTTP_401_UNAUTHORIZED, status.HTTP_403_FORBIDDEN}:
            reason = "unauthorized" if exc.status_code == status.HTTP_401_UNAUTHORIZED else "forbidden"
    ...
    if errand.status not in ACTIVE_TRACKING_STATUSES and errand.status not in {"completed", "cancelled"}:
    ...
    if errand.status in ACTIVE_TRACKING_STATUSES:
        ...
        if stale_loc:
            ...
            if distance_m <= STALL_DISTANCE_METERS:
                should_alert = not last_issue or (now - last_issue) > timedelta(minutes=STALL_REPEAT_MINUTES)
                if should_alert:
    ...
    if errand.status in ACTIVE_TRACKING_STATUSES and not errand.tracking_paused and window_info["within_window"]:
...
async def get_current_location(
    ...
    # Get latest location
    query = select(PilotLocation).where(
        PilotLocation.errand_id == errand_id
    ).order_by(desc(func.coalesce(PilotLocation.recorded_at, PilotLocation.created_at))).limit(1)
...
async def get_route_history(
    ...
    query = select(PilotLocation).where(
        PilotLocation.errand_id == errand_id
    ).order_by(func.coalesce(PilotLocation.recorded_at, PilotLocation.created_at), PilotLocation.id)
    ...
    duration = locations[-1].created_at - locations[0].created_at if len(locations) > 1 else None
...
async def get_locations_last_hour(
    ...
    query = select(PilotLocation).where(
        and_(
            PilotLocation.errand_id == errand_id,
            func.coalesce(PilotLocation.recorded_at, PilotLocation.created_at) >= one_hour_ago
        )
    ).order_by(func.coalesce(PilotLocation.recorded_at, PilotLocation.created_at), PilotLocation.id)
...
async def websocket_endpoint(websocket: WebSocket, errand_id: int):
    ...
    if not payload["tracking_allowed"] or errand.status not in ACTIVE_TRACKING_STATUSES:
        ...
        await websocket.send_json({
            "type": "tracking_unavailable",
            "reason": payload["reason"] or "Tracking is not active for this errand.",
        })
        await websocket.close(code=1008)
        return
...
def calculate_total_distance(locations: list[PilotLocation]) -> float:
    ...
    for i in range(len(locations) - 1):
        lat1, lon1 = radians(float(locations[i].latitude)), radians(float(locations[i].longitude))
        lat2, lon2 = radians(float(locations[i+1].latitude)), radians(float(locations[i+1].longitude))
