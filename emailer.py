...
def _format_from_email(from_email: str) -> str:
    from_name = (os.getenv("SMTP_FROM_NAME") or os.getenv("EMAIL_FROM_NAME") or "").strip()
    if not from_name:
...
def graph_health_check() -> HealthResult:
    ...
    try:
        with httpx.Client(timeout=timeout) as client:
            ...
            if not access_token:
                return HealthResult(ok=False, provider="graph", detail="missing_access_token")
...
def smtp_health_check() -> HealthResult:
    ...
    try:
        ...
        return HealthResult(ok=False, provider="smtp", detail=f"smtp_health_failed: {e}")
...
def send_email(*, to_email: str, subject: str, body_text: str) -> SendResult:
    ...
    if _graph_enabled():
        result = _send_via_graph(to_email=to_email, subject=subject, body_text=body_text)
        if result.delivered:
            return result
    ...
    if _smtp_enabled():
        ...
        if not result.delivered:
            ...
            if allow_fallback:
                ...
                print(body_text)
                print("--- END EMAIL ---\n")
                return SendResult(delivered=True, provider="stdout", detail="smtp_failed_logged")
        return result
...
def _send_via_graph(*, to_email: str, subject: str, body_text: str) -> SendResult:
    ...
    try:
        with httpx.Client(timeout=timeout) as client:
            ...
            access_token = token_res.json().get("access_token")
            if not access_token:
                return SendResult(delivered=False, provider="graph", detail="missing_access_token")
            ...
            if reply_to:
                mail_payload["message"]["replyTo"] = [
                    {"emailAddress": {"address": reply_to, "name": from_name or reply_to}}
                ]
            if from_name:
            ...
            sender_encoded = quote(sender)
            send_url = f"https://graph.microsoft.com/v1.0/users/{sender_encoded}/sendMail"
...
def _send_via_smtp(*, to_email: str, subject: str, body_text: str) -> SendResult:
    ...
    username = (os.getenv("SMTP_USERNAME") or "").strip() or None
    password = os.getenv("SMTP_PASSWORD")
    from_email = (os.getenv("SMTP_FROM") or "").strip() or username or "no-reply@errandbridge.com"
    use_tls = (os.getenv("SMTP_USE_TLS", "true").strip().lower() in ("1", "true", "yes"))
    timeout = int(os.getenv("SMTP_TIMEOUT_SECONDS", "10"))
    auth_mode = (os.getenv("SMTP_AUTH_MODE") or "login").strip().lower()
    fallback_from = None
    if username and from_email and from_email.strip().lower() != username.strip().lower():
    ...
    try:
        ...
        transient_retry = isinstance(e, (smtplib.SMTPServerDisconnected, TimeoutError, socket.timeout))
        if transient_retry:
            try:
                _attempt_send(from_email)
                return SendResult(delivered=True, provider="smtp", detail="retry_success")
        ...
        if fallback_from:
            ...
            try:
                ...
                return SendResult(delivered=True, provider="smtp", detail="fallback_from_username")
