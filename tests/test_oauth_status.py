...
@pytest.mark.asyncio
async def test_oauth_status_marks_google_redirect_disabled_when_secret_missing(
    monkeypatch,
):
...
@pytest.mark.asyncio
async def test_oauth_status_marks_google_token_enabled_when_client_id_present(
    monkeypatch,
):
    monkeypatch.delenv("OAUTH_ALLOWED_ORIGINS", raising=False)
    monkeypatch.delenv("CORS_ALLOW_ORIGINS", raising=False)
...
async def test_oauth_status_disables_redirect_flows_for_disallowed_origin(monkeypatch):
    ...
    monkeypatch.setenv(
        "APPLE_REDIRECT_URI", "https://api.errandbridge.com/auth/oauth/apple/callback"
    )
...
@pytest.mark.asyncio
async def test_oauth_status_allows_localhost_in_non_prod_even_with_explicit_origins(
    monkeypatch,
):
