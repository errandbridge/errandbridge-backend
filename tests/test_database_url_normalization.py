...
def test_normalize_async_from_postgres_scheme():
    ...
    assert database._normalize_database_url(url, mode="async").startswith("postgresql+asyncpg://")
...
def test_normalize_sync_from_postgresql_scheme():
    url = "postgresql://user:pass@host:5432/db"
    assert database._normalize_database_url(url, mode="sync").startswith("postgresql+psycopg://")
