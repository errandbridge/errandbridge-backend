...
class PilotLocation(Base):
    ...
    errand_id = Column(Integer, ForeignKey("errands.id", ondelete="CASCADE"), nullable=False, index=True)
    ...
    source = Column(String, nullable=False, server_default="mobile_app")  # mobile_app | fob
    recorded_at = Column(DateTime(timezone=True), nullable=True)  # device-recorded event time
