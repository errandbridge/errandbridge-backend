...
async def seed_data():
    async with AsyncSessionLocal() as session:
        ...
        errands = [
            Errand(
                user_id=customer.id,
                reference_number=_make_reference_number(int(time.time() * 1_000_000) + 1),
            ...
            Errand(
                user_id=customer.id,
                reference_number=_make_reference_number(int(time.time() * 1_000_000) + 2),
            ...
            Errand(
                user_id=customer.id,
                reference_number=_make_reference_number(int(time.time() * 1_000_000) + 3),
