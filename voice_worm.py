...
def _config() -> WormConfig | None:
    ...
    return WormConfig(bucket=bucket, prefix=prefix, retention_days=retention_days, region=region)
...
def store_worm_bytes(*, key_suffix: str, content: bytes, content_type: str) -> Optional[str]:
    cfg = _config()
    if cfg is None:
        return None
