...
def http_get(url: str, timeout: float = 10.0, headers: dict[str, str] | None = None) -> HttpResult:
...
def graphql(base_url: str, query: str, variables: dict[str, Any] | None = None) -> dict[str, Any]:
...
def main() -> int:
    ...
    assert "ErrandBridge" in root.body_text, "Expected 'ErrandBridge' marker in root response"

    print("[smoke-no-otp] GET /metrics")
    metrics = http_get(f"{base_url}/metrics")
    assert metrics.status == 200, f"Expected 200, got {metrics.status}: {metrics.body_text[:250]}"
    assert "# HELP" in metrics.body_text and "# TYPE" in metrics.body_text, "Expected Prometheus exposition format"

    print("[smoke-no-otp] GraphQL: schema accepts confirmationSentAt")
    data = graphql(base_url, "query { errands { id confirmationSentAt } }")
    assert "errands" in data and isinstance(data["errands"], list), "Expected errands list"
    ...
    if admin_token:
        ...
        try:
            ...
            raise AssertionError(f"Expected JSON body from /admin/errands but failed to parse: {e}\n{res.body_text}")

        assert isinstance(body, list), "Expected list response from /admin/errands"
        if body:
            sample = body[0]
            assert "reference_number" in sample, "Expected reference_number in admin errand item"
            assert "confirmation_sent_at" in sample, "Expected confirmation_sent_at in admin errand item"
        print("[smoke-no-otp] /admin/errands OK")
    else:
        print("[smoke-no-otp] skipping /admin/errands (set ADMIN_BEARER_TOKEN to enable)")
