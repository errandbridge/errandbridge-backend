...
raw_url = os.getenv("DATABASE_URL", "postgresql+asyncpg://postgres:postgres@db:5432/errandbridge")
...
with engine.begin() as conn:
    ...
    before = conn.execute(text("SELECT password_hash FROM users WHERE email=:email"), {"email": email}).fetchone()
    ...
    after = conn.execute(text("SELECT password_hash FROM users WHERE email=:email"), {"email": email}).fetchone()
