...
def _voice_enabled() -> bool:
    return bool(
        ...
        and ((os.getenv("TWILIO_VOICE_NUMBER") or "").strip() or (os.getenv("TWILIO_FROM_NUMBER") or "").strip())
    )
...
def _voice_config() -> VoiceConfig | None:
    ...
    return VoiceConfig(
        ...
        voice_number=(os.getenv("TWILIO_VOICE_NUMBER") or os.getenv("TWILIO_FROM_NUMBER") or "").strip(),
    )


def create_call(*, to_number: str, twiml_url: str, status_callback: str | None = None) -> VoiceCallResult:
...
def request_transcription(*, recording_sid: str, callback_url: str | None = None) -> VoiceCallResult:
    config = _voice_config()
    if config is None:
        return VoiceCallResult(ok=False, detail="missing_config")
    if Client is None:
